 ...
 */
export function PlayerSkeleton({ still = false }: { still?: boolean }) {
    ...
    <div aria-hidden>
      <div className="-mx-4 flex items-center justify-between gap-2 border-b border-border/60 px-4 py-2.5">
        <div className="flex items-center gap-2">
          <Skeleton className={cn('h-5 w-24 rounded', bar)} />
          <Skeleton className={cn('hidden h-5 w-28 rounded-full sm:block', bar)} />
        </div>
        <div className="flex items-center gap-1.5">
          <Skeleton className={cn('h-7 w-24 rounded-full', bar)} />
          <Skeleton className={cn('h-8 w-16 rounded-md', bar)} />
        </div>
      </div>

      <div className="pt-6">
        <div className="space-y-2">
          <Skeleton className={cn('h-6 w-full rounded', bar)} />
          <Skeleton className={cn('h-6 w-2/3 rounded', bar)} />
        </div>
        <div className="mt-6 flex flex-col gap-2.5">
          {[1, 0.85, 0.65, 0.45].map((opacity, index) => (
            <Skeleton
              key={index}
              className={cn('h-[62px] w-full rounded-xl', bar)}
              style={{ opacity }}
            />
          ))}
        </div>
 ...
 * the skeleton, held STILL, in the play column. This boundary covers an RSC
 * payload rather than an API request, so it reserves the shape without claiming
