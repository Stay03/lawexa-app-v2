...
/**
 * StudyHome — the Study tab's home surface (owner #34). An HONEST MINIMUM this
 * wave: the shared greeting + composer + suggested prompts, composed like the
 * Chat home so it reads as a complete, intentional surface the moment the owner
 * flips to it — just QUIETER. Wave T3 fills this tab with its real modules (study
 * spaces with badges, Quiz continue + stats strip + topics, the student-gated
 * study-mode CTA, recent bookmarks, conversations strip). None of those load yet,
 * so there is deliberately NO module scaffolding and NO skeletons here — a
 * skeleton would imply something is loading; nothing is. The tab's PURPOSE is
 * hinted only by a quiet greeting subline, not by placeholder furniture.
 *
 ...
 *
 * Carries `data-home-tab="study"` for the tab wrapper and the server-renderable
  ...
  return (
    ...
    >
      {/* Greeting — Comfortaa, capped at v1's ~36px. The quiet subline hints the
          tab's purpose (revision + saved work) without promising furniture that
