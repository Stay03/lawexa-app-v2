...
import type { ViewsByCountryPoint } from '@/types/admin';
import { getCountryCode } from '@/lib/constants/country-codes';

interface BotViewsByCountryChartProps {
  data: ViewsByCountryPoint[];
}

const BAR_COLORS = ['#3b82f6', '#a855f7', '#22c55e', '#f97316', '#ef4444'];

export function BotViewsByCountryChart({ data }: BotViewsByCountryChartProps) {
  if (!data.length) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Bot Crawls by Country</CardTitle>
          <CardDescription>Where bot crawlers connect from (often datacenters)</CardDescription>
        </CardHeader>
