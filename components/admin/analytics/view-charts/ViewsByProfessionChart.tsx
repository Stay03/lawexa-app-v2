...
import type { ViewsByProfessionPoint } from '@/types/admin';

interface ViewsByProfessionChartProps {
  data: ViewsByProfessionPoint[];
}
...
export function ViewsByProfessionChart({ data }: ViewsByProfessionChartProps) {
  if (!data.length) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Views by Profession</CardTitle>
          <CardDescription>Viewer breakdown by profession</CardDescription>
        </CardHeader>
  ...
  const topProfession = data.reduce((max, item) => (item.count > max.count ? item : max), data[0]);
                  ...
                  <ChartTooltipContent
                    ...
                    formatter={(value, name) => {
                      ...
                      return [
                        ...
                        formatProfession(String(name)),
                      ];
                    }}
                  />
                }
              />
              <Pie data={chartData} dataKey="count" nameKey="profession" innerRadius={38} outerRadius={55} strokeWidth={2}>
                {chartData.map((entry) => (
          ...
          <div className="flex flex-1 flex-col gap-2.5">
            {chartData.map((item) => (
              <div key={item.profession} className="flex items-center gap-2">
                ...
                <span className="flex-1 text-sm text-foreground">{formatProfession(item.profession)}</span>
